...
\begin{figure}
\caption{A broad phylogenetic classification of Yolŋu subgroups, following \citealt{Wilkinson1991,Schebeck2001,Waters1989} a.o. with some adaptation following \citealt{BowernAL} (ed.) treats ``\textsc{western}'' as belonging to a \textsc{northern} clade (forthcoming: x).}\centering\label{YM-phylo}
...
\begin{figure}[h]\centering	\caption{Varieties (`clanlects'/dialects) of \textcolor{teal}{Dhuwal}-\textcolor{ochre}{Dhuwala} in the context of the Southern Yolŋu languages \citep[following][13]{Wilkinson1991} with some adaptation following \citet[15]{Schebeck2001} and \citet{BowernAL} (ed.) who does not claim that \textsc{southern} and \textsc{central} forma  single clade (forthcoming: x).}\label{DDvars}
...
\begin{figure}[H]
	\caption{The contribution and licensing of \gls{IV} in negative contexts: a branching times schema of (\lastx)}\label{fig:negderiv}
{\centering
\begin{tikzpicture}[scale=1.5]
...
\addcontentsline{toc}{subsubsection}{What does a negative sentence presuppose? : Polarity “asymmetricalism”}
...
\section[\textsc{\textit{MaxPresupp 2}}: The same-day future]{\textsc{Maximize Presupposition} again:\\The same-day future}\label{yolngu-sdf}
...
\chapter{The \gls{wd} paradigm}\label{ch:paradigm}
...
\pex \textbf{Domains of the four inflections in \gls{wd}, given a branching time frame 
	$ \mathfrak U =\langle\mathcal I,\prec\rangle$ and an evaluation context $ c $ }\label{wd-doms}\par\nobreak
\quad\vbox{\halign{%
		#\hfil&  #\hfil&& \hskip1em #\hfil\cr
		%& \hfil\hwit{root}& \hfil\hwit{perfect stem}&
		%\hfil\hwit{gloss}\cr
...
\xe
 ...
 \begin{figure}[h]
 	\caption{\textsc{\textbf{MaxPresupPoset}} $ \langle \textsc{infl},\Rightarrow\rangle $: Blocking relations between the inflectional categories. Given a reference index $ i_c $, speakers select the form with the most specific presuppositions that can be satisfied in context.}\label{fig:poset}\centering
...
\begin{figure}[h]
	\centering
	\caption[Domains of the WD inflectional categories]{The \gls{wd} inflectional categories are modelled as partial identity functions over $ \mathcal I $. The (approximate) domains of each are projected over a Branching Times frame, where $ i* $ is the evaluation index.}
	\label{fig:wd-BT}
...
\fi
	...
	Below we briefly consider data with apparent epistemic modal expressions in view of fine-tuning our notion of \textsc{nonveridicality} and considering how well the above-mentioned characterisation of  \textit{irrealis} as nonassertoric fits the \gls{wd} data.
...
A precise formulation of this operator's (these operators') semantics is not necessary for current purposes; what follows (\nextx) represents a rough proposal in view of clarifying the nature of the \textsc{metaphysical/objective nonveridicality} property described above.
	...
	=\lambda p\lambda i.\cap{\sim_\alpha i} \subseteq p$
	
	%$\denote{\textsc{assert}} = \lambda p\lambda w.\forall w'[w'\in\mathcal B_{\text{Spkr}_c} (w)\to p(w')]$\\
	$ \sim $ is an accessibility relation that, given a speech index $ i $ returns all the propositions that a given judge $ \alpha $ will publicly commit to at that index.
	...
	The force of this modal can additionally be weakened by epistemic possibility adverb \textit{mak(u)}. For \citeauthor{Krifka2021}, epistemic adverbs modify the level to which a given judge is certain about/willing to commit to the truth of a given proposition \citeyearpar[12]{Krifka2021}. Given its apparent variable modal force, \textit{maku} takes an accessibility relation (\textit{e.g.}, \textsc{assert}) as its sister and returns a subset of the modal base it picks out.  Following \citealt{Matthewson2010,Rullmann2008} a.o., force-variable modality is modelled as universal quantification over a a (contextually-determined) subset of the modal base (as determined by a ``contextually given'' choice function $ f_c $.) The size of the output of $ f_c $ is proportional to the strength of the assertion. \textit{maku}-modified assertions are therefore also compatible ``with cases that introduce a proposition and its negation'' \citetext{\citealp[13]{Krifka2021}, compare (\getfullref{mak.raŋi}) above.}
...
\pex\deftagex{ccls} \textbf{Matrix predicates which entail nonveridical complements do not license the irrealis}
...
\subsection{Locality \& \textit{objective nonveridicality}}
...
Broadly speaking, given an utterance index, \textit{dhu} `\gls{fut}' displaces a predicate into the \textsl{potential} domain, \textit{bäyŋu/yaka} into the \textsl{counterfactual domain} and \textit{balaŋ} into either of these (the ``\textsl{irrealis} domain,'' following \citealp{VonPrincea} forthcoming.)
...
The irrelevance of epistemic/judgment modification to \gls{irr} is in fact compatible with the definition given in (\ref{def:IRR}) above, which features in the denotations for the \gls{irr} mood inflections, realised as \gls{II} and \gls{IV}. As shown there, \textsc{irr} mood is concerned with the availability of some \textit{metaphysical alternative} ($ b\in\cap{\approx_{i'}} $). When making a predication about some index preceding the utterance ($ i'\in\cap{\preceq}i* $) any it is presupposed that the facts about any given \textit{P} are settled/uniform across metaphysical alternatives (``the fixity of the past''.) Conversely, when making a predication about some $ i'\notin\cap{\preceq}i* $: \textit{i.e.}, the \textit{irrealis} domain (\citealp{VonPrincea} forthcoming), $ \cap{\approx_{i'}} $ are \textbf{diverse}

 \textit{i.e.}, \textbf{objective} (as opposed to
...
A proper treatment of subordination is a matter for future work, although the effects (or lack thereof) of nonfactive predicates on mood inflection in complement clauses appears to be unifiable with the current analysis. Theories of reference generally assume a rule that matrix clauses are evaluated with respect to the parameters of a given utterance. \textit{I.e.}, the time, world, location, discourse participants \textit{etc.} are \textbf{automatically} set by/identified with the utterance context.

As described above, \gls{irr} seems to be licensed (in matrix clauses) when there is a metaphysical alternative, as evaluated at the reference index $ (i_c) $ along which \textit{P} doesn't hold. The common ground, modellable as $ \cap{\approx_{i*}} $ (\textit{i.e.}, metaphysical alternatives to the utterance index)\footnote{In Stalnaker's terms, this specifies the  \textit{context set}: the set of worlds returned by intersecting all propositions in the common ground.} 



\section{The proposal in action}
