...
\marginnote{In neither the LSP volume (contribution objectives) and the NELS proceedings (space) do I give a deeper, typologist-style overview of the NEC, slowing down and showing what's going on at each stage. Could do this reasonably quickly here (the FodS/ LSA talks do give this.)}[-2in]
...
\pex\begingl\glpreamble \textit{Preverbal standard negation in Nakkara}\trailingcitation{\citep[191]{Eather2011}} \deftagex{sn-nck}//
...
\pex\deftagex{sn-wyb}\textit{ Preverbal standard negation in Ngiyambaa}\trailingcitation{\citep[239]{Donaldson1980}}
...
\pex\textit{Function of \textit{-majirri} `\gls{priv}' in Nyangumarta}\deftagex{nna-privEx}\trailingcitation{\citep[140]{Sharp2004}}
...
\pex \textit{Function of \textit{-kil} `\gls{priv}' in Muruwarii}\trailingcitation{\citep[77-8]{Oates1988} }
...
\begin{table}[H]\centering
	\caption{Reported partitions in the negative semantic space (data adapted from \citealt{Hercus1999,Hercus1992,Schurmann1844,Hercus1996,Black1917}.) Colouring reflects hypothesised cognacy of lexical items across Thura-Yura. Dashed arrows represent borrowings from neighbouring languages, solid arrows semantic (functional) change.} \label{TYdata}
...
\citealt[225]{Miestamo2005}, \citealt{Phillips2021b}.)\marginnote{there's sth strange going on w the T\&S sourcing here, i'm not totally sure who says what \& may need to go back to archives to find out.}
...
\glft\textbf{Intended reading:} `Because there's no meat for us.'\trailingcitation{(\citealp[560]{Wilkinson1991}, infelicity judgment \textsc{aw20190505}, cf. \lastx c)}//\endgl\deftagex{djr-bayŋu}
\xe
...
If this is indeed the case, the trajectory runs counter to hypotheses of a unidirectional NEC (\textit{e.g.}, \citealt[146]{Veselinova2016}): an innovative \textit{standard negator} has been recruited into Ritharrŋu's negative space, whereas the so-called ``special negators'' have retained an older form (Figure \ref{RitNEC}).
...
\pex The status of negative inflection in Eastern/Central varieties of Arrernte
...
\Tree [.**pre-p-Arandic \edge node[auto=left]{\footnotesize**$\mathcal{B\to C}$}; [.*p-Arandic
\textit{Kaytetye} \edge node[auto=left]{\footnotesize*$\mathcal{C\to B^\prime}$};
[.core~Arrente ] ] ]
\end{tikzpicture}\end{subfigure}
\begin{subfigure}{.68\textwidth}
	\begin{enumerate}[\bf i]
		\item By hypothesis, pre-proto-Arandic conforms with `standard average Australian' preverbal SN strategies with a distinct post-nominal privative (\textit{**kenhe})\trailingcitation{$\boldsymbol{\mathcal B}$ }
...
\chapter{The NƎC and a unified semantics}\label{disc}\label{NEC2}
...
\pex Incomplete generalisation of \textit{bäyŋu} \gls{negex} in Djambarrpuyŋu (repeated from (\getref{djr-yaka}-\getref{djr-bayŋu}) above\deftagex{yaka-bayŋu})\trailingcitation{[AW~20190505]}
...
\xe
\pex \a\begingl\gla maŋutji ŋorranha\textdblhyphen{miriw} ŋunha-yi wäŋa\deftagex{sleep}\deftaglabel{gl}//
...
\xe
	\pex
	 \a\begingl\gla ḻukanha\textdblhyphen{miriw} ŋayi ŋunhi dharpa-ny\deftagex{tree}\deftaglabel{gl}//
...
Further, as (\nextx) shows, the GQ-based analysis presented here correctly predicts the unavailability of a reading where the apparent modal operator is outscoped. Where (\getref{bathi.bay}) exhibits scopal ambiguity, a reading where the modal component (supplied by $ \mathcal R $) outscopes negation is not found in (\getref{bathi.priv}).\footnote{See \citet[Ch. 5]{Horn2001} for a discussion of the properties of affixal/incorporated negative elements} A few additional observations about constraints on eventive \textit{-miriw} are given in \S~\ref{codas}.
	...
	$ \denote{\textup{\getfullref{bathi.priv}}}=\textbf{no}\big(\lambda e.\textbf{sink}(e),\lambda e'.\textbf{char}(\mathit{bathi},e')\big) $
...
The meaning change described by the NEC seems, then, to be associated with a concomitant loss in \textit{discretionary indexicality}. On the quantificational (modal) analysis of negation described in the previous section, the meaning contribution of a sentential negator is that its prejacent --- $ p\in\wp(\mathcal W) $) --- \textit{does not intersect} with the set of worlds which are \textit{compatible} with the actual world $ \lambda w'.\mathbb C(w\!*,w') $. That is, the establishment of reference is automatically set and speaker meaning (the hallmark of discretionary indexicality) isn't factored in.
...
As shown in the discussion of the Yolŋu privative (\S~\ref{sec:evpriv}) \textit{-miriw} appears to be attach to an entire nominalised (event-denoting) verb phrase, suggesting the reanalysis of this form as ``phrasal morphology'' (\textit{i.e.}, a special clitic, \citealp[see][]{Anderson2005}.) Events of the type described by the privative phrase then are then taken to be related (by $ \mathcal R $) to some set of events associated with the coda (which is realised as grammatical subject). Contextually-retrieived $ \mathcal R $ is locus of the (pragmatically ambiguous) modal reading of propositions containing an eventive-privative. What's more, there are a number of constrants on the types of relations that are made available.
...
\xe\marginnote{constraints on permissible coda-pivot relations/thematic roles obviously aren't worked out here. Can keep trying or drop this.}
 ...
 In Arrernte, where \textit{-tye\textdblhyphen{kenhe}} has completed the \gls{priv} $ \to $ \gls{neg} change, remaining morrphosemantic constraints on the syntactic unit to which attaches appear to be removed. 
